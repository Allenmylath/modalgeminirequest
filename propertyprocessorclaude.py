...
image = modal.Image.debian_slim().pip_install(
    [
        ...
        "httpx",
        "pybase64",
        "anthropic",  # Updated package name
        "python-dotenv",
        "fastapi[standard]",
    ]
)
...
class PropertyImageProcessor:
    def __init__(
        ...
        rate_limit: int = 50,  # Claude has different rate limits
        max_retries: int = 3,
        max_size_mb: float = 50.0,
        max_concurrent: int = 20,  # Conservative for Claude API
        max_tokens: int = 4096,
    ):
        ...
        if not self.claude_api_key:
            raise ValueError(
                "ANTHROPIC_API_KEY must be provided either as parameter or in environment variable"
            )
    ...
    async def process_single_property(
        ...
        try:
            ...
            description = await self._call_claude_api(
                ...
                property_data["images"][:images_processed_count],
            )
    ...
    async def _call_claude_api(
        ...
        """Call Claude API with retry logic, timeout, and rate limiting using mixed content format."""
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Create Claude-style content array with mixed text and images
                content = []

                # Add initial prompt
                content.append({"type": "text", "text": full_prompt})

                # Add images with labels (following the reference pattern)
                for i, (img_b64, img_url) in enumerate(
                    zip(images_base64, original_image_urls), 1
                ):
                    # Add image label
                ...
                response = await asyncio.wait_for(
                    loop.run_in_executor(None, self._sync_claude_call, content),
                    timeout=300.0,
                )
                ...
                if attempt < self.max_retries:
                    ...
                    self.logger.error(f"All Claude API attempts failed: {e}")
                    raise e

    def _sync_claude_call(self, content):
        """Synchronous Claude API call to be run in executor."""
    ...
    async def _enforce_rate_limit(self):
        """Rate limiting: 50 requests per minute for Claude (more conservative)."""
    ...
    async def collect_and_format_results(
        ...
        results = {
            "analysis_metadata": {
                ...
                "claude_model_used": self.claude_model,
